...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (document.getElementById("status").value !== "Completed" && currentDate > deadlineDate) {
            document.getElementById("status").value = "Overdue";
            alert("Task is overdue! Status updated to Overdue.");
        }
    });
</script>
...
<a href="#" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
   class="list-group-item list-group-item-action bg-transparent text-danger fw-bold">
    ...
    <h1 class="text-center"><b>Update Task Status</b></h1>

    

    <div class="form-group">
        <label for="task_name">Task Name</label>
        <input type="text" class="form-control" id="task_name" name="task_name" value="{{ $task->task_name }}" required readonly>
    </div>

    <div class="form-group">
        <label for="description">Description</label>
        <textarea class="form-control" id="description" name="description" rows="4" required readonly>{{ $task->description }}</textarea>
    </div>

    <div class="form-group">
        <label for="deadline">Deadline</label>
        <input type="date" class="form-control" id="deadline" name="deadline" value="{{ $task->deadline }}" required readonly>
    </div>
